# Simple home automation
A collection of examples and information for a simple and affordable home automation solution.

![GitHub](https://img.shields.io/github/license/norman-de/simple-home-automation) ![GitHub last commit](https://img.shields.io/github/last-commit/norman-de/simple-home-automation)

## Overview of used tools

### Hardware
* [Raspberry Pi - small single-board computers](https://www.raspberrypi.org/products/raspberry-pi-zero-w/)
    * [Node Red - flow-based development tool for visual programming](https://nodered.org/)
    * [Eclipse Mosquitto™ - open source message broker](https://mosquitto.org/)
* [Tasmota - Alternative firmware for ESP8266 based devices ](https://github.com/arendst/Tasmota)
    * [ WEMOS D1 mini - A mini wifi board with 4MB flash based on ESP-8266EX](https://github.com/arendst/Tasmota/wiki/Wemos-D1-Mini)
    * [OBI Socket 2 - cheap socket from the German Hardware Store](https://github.com/arendst/Tasmota/wiki/OBI-Socket-2)
* [Particle Photon - tiny Wi-Fi IoT device](https://docs.particle.io/photon/) 
* [TV Stick with RTL2832U Chip](https://www.idealo.de/preisvergleich/MainSearchProductCategory.html?q=RTL2832U)

### Software
* [MQTT Dash - Android App for MQTT dashboards](https://play.google.com/store/apps/details?id=net.routix.mqttdash&hl=de)
* [mqtt-spy -  open source utilities for publishing and monitoring activity on MQTT topics.](https://github.com/eclipse/paho.mqtt-spy/wiki/Overview)
* [rtl_433 -  generic data receiver, mainly for the 433.92 MHz, 868 MHz (SRD), 315 MHz, and 915 MHz ISM bands](git://github.com/merbanan/rtl_433)
* [thingsboard.io - open source IoT Plattform](https://thingsboard.io/)
    * [ThingsBoard IoT Gateway](https://thingsboard.io/docs/iot-gateway/)

###  Overview chart
![overview](img/overview.png?raw=true)

### Devices overview
* [Electric heating control with Adafruit AM2302 temperature & humidity sensors](https://github.com/norman-de/simple-home-automation/wiki/electric-heating-control)
* [Battery powered MQTT WIFI remote control for scene controlled light](https://github.com/norman-de/simple-home-automation/wiki/MQTT-WIFI-remote-control)
* [Plant soil moisture sensor with TSL45315 I2C light sensor](https://github.com/norman-de/simple-home-automation/wiki/plant-soil-moisture-sensor)
* [Timed light for pets](https://github.com/norman-de/simple-home-automation/wiki/timed-light-for-pets)
* [Water level detector for dehumidifiers with hc-sr04 ultrasonic sensor and Si7021 I2C humidity and temperature sensor](https://github.com/norman-de/simple-home-automation/wiki/water-level-detector)
* Weather info display with Nextion TFT HMI display NX4827T043
* Sensor values from Oregon Scientific Multi-zone Weather Station with SDR
